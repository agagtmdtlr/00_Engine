...
void Model::ReadMesh(wstring file)
{
	...
	for (UINT i = 0; i < count; i++)
	{
		...
		{			
			...
			r->Byte(&ptr, sizeof(Model::ModelVertex) * count); // byte read


			mesh->vertices = new Model::ModelVertex[count]; // vertices crate
...
void Model::BindMesh()
{
	for (ModelMesh* mesh : meshes) // 각 메쉬마다 바인드해줄 데이터를 처리한다. ex(bone, material)
	{
