...
void Collider::Render(Color color)
{
	...
	for (UINT i = 0; i < 8; i++)
	{
		// 충돌체의 정점을 지정된 월드 변환 매트릭스로 이동시키기
...
bool Collider::Intersection(Ray & ray, float * outDistance)
{
	...
	*outDistance = 0.0f;
	...
	// 나누기에 사용할거라 0이 아닌값으로 변환 정규화에 사용한다.
	...
	if (fabsf(ray.Direction.x) >= 1e-6f)
	{
		...
		float value = 1.0f / ray.Direction.x;// 각축의 거리는 방향따른 비례량을을 고려하여 정규화 시켜서 모든 축에 대해서 거리를 비교하기 위함

		float minX = (minPosition.x - ray.Position.x) * value;
		float maxX = (maxPosition.x - ray.Position.x) * value;
		// x축 방향으로 멀리 떨어진 거리
		// x축 방향으로 가까이 떨어지 거리 구하기
