...
void Texture::SaveFile(wstring file, ID3D11Texture2D * src)
{
	...
	if(ext == L"DDS")
		hr = D3DX11SaveTextureToFile(D3D::GetDC(), dest, D3DX11_IFF_DDS, file.c_str());
	else
...
void Textures::Load(Texture * texture, D3DX11_IMAGE_LOAD_INFO * loadInfo)
{
	...
	{
		hr = GetMetadataFromHDRFile(texture->file.c_str(), metaData);
		Check(hr);
		//assert(false);
	}
	else
	...
	UINT mipmap = metaData.mipLevels;
	...
	desc.width = width;
	desc.height = height;
	desc.metaData.mipLevels = mipmap;
	...
	else
	{
		...
		{
			hr = LoadFromHDRFile(texture->file.c_str(), &metaData, image);
			Check(hr);
			//assert(false);
		}
		else
