...
void Converter::ReadFileDiffMesh(wstring file)
{
	// 파일은 저장할고 쓸 경로
...
void Converter::ExportStaticMesh(wstring savePath)
{
	// 우리가 저장할 형식은 .mesh 확장좌.
...
void Converter::ExportMeshDiff(wstring savePath)
{
	//savePath = L"../../_Models/" + savePath + L".mesh";
...
void Converter::WriteStaticMeshData(wstring savePath)
{
	Path::CreateFolders(Path::GetDirectoryName(savePath));
...
void Converter::DiffReadBoneData(aiNode * node, int index, int parent)
{
	asBone* bone = new asBone();
	bone->Index = index;
	bone->Parent = parent;
...
void Converter::DiffReadMeshData(aiNode * node, int bone)
{
}

void Converter::DiffReadSkinData()
{
	for (UINT i = 0; i < scene->mNumMeshes; i++)
	{
		aiMesh* aiMesh = scene->mMeshes[i]; // 메쉬마다 처리
...
void Converter::DiffWriteMeshData(wstring savePath)
{
}

Matrix Converter::DiffMatrix(aiNode * node, string name)
{
	Matrix result;
...
void Converter::ExportMaterialPBR(wstring savePath, bool bOverwrite)
{
...
void Converter::ReadMaterialDataPBR()
{
	for (UINT i = 0; i < scene->mNumMaterials; i++)
	{
		aiMaterial* srcMaterial = scene->mMaterials[i];
...
void Converter::WriteMaterialDataPBR(wstring savePath)
{
	string folder = String::ToString(Path::GetDirectoryName(savePath));
	// Path::GetFileName :: 확장자 포함한 파일명을 가져온다.
	// Path::GetFileNameWithoutExtension : 확장자 제외한 파일명을 가져온다.
